...
def _ref_id(fields: dict) -> str:
    seed = (
        fields.get("sec_accession")
        or fields.get("doi")
        or fields.get("pmid")
        or norm_url(fields.get("url", ""))
...
def upsert_reference(fields: dict, con) -> tuple[str, bool]:
    ...
    if row["ref_id"] in have:  # seed collision on different identifiers
        row["ref_id"] = (
            "R" + hashlib.sha256((row["ref_id"] + row["title"] + _now()).encode()).hexdigest()[:16]
        )
...
def add_link(
    ref_id: str, key_type: str, entity_key: str, role: str, con, added_by: str = ADDED_BY_DEFAULT
) -> bool:
    rows = (
    ...
    if any(
        r["ref_id"] == ref_id
        and r["key_type"] == key_type
        and r["entity_key"] == entity_key
        and r["role"] == role
        for r in rows
    ):
        return False
...
def manifest(con=None) -> Path:
    ...
    for p in sorted(library_root().rglob("*")):
        if p.is_file() and p != out:  # the manifest never lists itself (fix 2026-08-31)
...
def merge_from(folder: Path, con=None) -> None:
    ...
    if refs_csv.exists():
        ...
        for r in crows:
            ...
            if p.exists():
                new_caps += int(
                    add_capture(
                        rid, r["capture_id"], p, r["kind"], r["capture_method"] or "merge", con
                    )
                )
    ...
    _ledger(
        f"library merge {folder}",
        {"folder": str(folder)},
        {
            "files_copied": copied,
            "references_new": new_refs,
            "captures_new": new_caps,
            "links_new": new_links,
        },
    )
    print(
        f"merge: {copied} files copied, {new_refs} references, "
        ...
        if r["ref_id"].startswith(ref_prefix):
            ...
            for c in store.read_table("captures", con=con):
                if c["ref_id"] == r["ref_id"]:
                    print(f"  capture {c['capture_id'][:12]} {c['kind']} {c['status']} {c['path']}")
...
def open_ref(ref_prefix: str, con=None) -> None:
    con = con or store.connect()
    for c in store.read_table("captures", con=con):
        if (c["ref_id"].startswith(ref_prefix) or c["capture_id"].startswith(ref_prefix)) and c[
            "status"
        ] == "active":
    ...
    for r in find_rows(con, entity=entity):
        for c in caps:
            if c["ref_id"] == r["ref_id"] and c["status"] == "active":
                ...
                name = f"{r['published_at'] or 'nodate'}_{r['ref_type']}_{safe}.{c['ext']}".replace(
                    " ", "_"
                )
...
def site(con=None) -> Path:
    ...
    for c in caps:
        if c["status"] == "active":
            cap_by_ref.setdefault(c["ref_id"], []).append(
                {
                    "path": ("../../" + "bronze/library/" if False else "") + c["path"],
                    "kind": c["kind"],
                }
            )
    data = [
    ...
    page = (
        "<!doctype html><meta charset='utf-8'><title>Research library</title>"
...
def cli(argv: list[str]) -> int:
        ...
        if not store.has_table(t, con):
            store.write_table(
                ...
                getattr(
                    schema,
                    {
                        "references": "REFERENCE_COLS",
                        "captures": "CAPTURE_COLS",
                        "reference_links": "REFERENCE_LINK_COLS",
                    }[t],
                ),
                con=con,
            )
    ...
    if sub == "find":
        for r in find_rows(
            con,
            entity=_one(flags, "entity"),
            ref_type=_one(flags, "type"),
            text=_one(flags, "text"),
            publisher=_one(flags, "publisher"),
            since=_one(flags, "since"),
        ):
            print(
                f"{r['ref_id'][:12]}  {r['published_at'] or '----------'}  "
    ...
    if sub == "dedupe":
        ...
        print(
            f"{len(cands)} candidate pairs (confirm with: library dedupe "
