...
def main(argv):
    ...
    if cmd == "add":
        ...
        if not rows:
            print("Nothing yet. Run:  python -m biointel trials", iid); return 1
        ...
        rows = build_snapshot()
        if not rows:
            print("No financials yet. Run:  python -m biointel fin-all"); return 1
        ...
        if not config.PARTNERS_CSV.exists():
            print("Run:  python -m biointel partners"); return 1
        ...
        if not config.DEALS_CSV.exists():
            print("Run:  python -m biointel deals-all"); return 1
