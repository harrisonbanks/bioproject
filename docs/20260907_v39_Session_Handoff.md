# docs/20260907_v39_Session_Handoff.md — paste this as the first message of the new chat

**Project:** Bioindustry Intelligence Platform. Repo `harrisonbanks/bioproject`
(private), branch `jason/refactor`, HEAD = `6644e4a` or a later commit the
...
1. Clone/pull; `git --no-pager log --oneline -1` must show `6644e4a` or a
...
3. Prove `PYTHONPATH=src pytest -q tests/unit` -> expect **251 passed**,
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.18, this handoff,
...
**THE FIRST TASK IS THE GATE M3 SCOPE MESSAGE** (standing judge queue).
The GATEM doc's direction is approved but each sub-gate needs its own
scope -> operator "go" -> build. Do not code before the go. M3 open
decision to settle in the scope: whether `unsure` rows ever expire
(GATEM proposal: no expiry, consistent with the hypothesis-store
decision).
...
1. **M3 — standing judge queue** (GATEM doc): disputed and disagreeing
   rows accumulate without blocking; verdicts land whenever, stamped
   as_of; correct promotes, wrong corrects from the header/document with
   the correction recorded, unsure stays excluded with the reason;
   `stakes precision` recomputes per rule version; exit: a row judged
   months late equals one judged same-day, proven on a replica. Scope
   message first; settle the no-expiry decision in it.
...
6. **M4 (LLM judge-assist)** — own scope-go-build after M3; proposer
   only, never the verdict; acceptance gate against the 148 judged rows
   before trust. Open decisions: model choice and per-run cost cap
