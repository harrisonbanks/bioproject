...
0e. docs/20260831_v14_Implementation_Plan.md — gate ledger and procedure; Phase 0, L1 and 1.4 DONE; order of record §3: L1 → 1.4 → L2 → 2.10 → 1.5 → L3 → L4, with 1.6, 1.7 and 2.9′ placed as they fall due; next gate L2 (seed via verified-overlay pattern, decided); §4 snapshot of record; §5 thirteen-file baseline; §6 legacy register; §7 model naming.
...
3. **Gate L2 scope message (next).** Dossier schema per Ontology v5 §3.10 and the §3.2/§3.3 attributes; dependency resolved 2026-08-31: seed through a committed hand-curated seed (verified-overlay pattern), 2.10 immediately after L2. Inside the L2 gate, in order: (a) the one-query Tempus/Personalis universe-membership check (Ontology §8 Q9; also settles whether the seed hand-adds an `ma_events` index row); (b) a library-capture block for the deal's source documents (operator holds the URLs; doc_ids come back in the paste and the seed module is written against them, so no placeholder ever enters a block); (c) schema + seed delivery. Q10 (transcript sources) remains open.
