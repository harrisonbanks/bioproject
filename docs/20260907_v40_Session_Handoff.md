# docs/20260907_v40_Session_Handoff.md — paste this as the first message of the new chat

**Project:** Bioindustry Intelligence Platform. Repo `harrisonbanks/bioproject`
(private), branch `jason/refactor`, HEAD = `099ccc8` or a later commit the
...
1. Clone/pull; `git --no-pager log --oneline -1` must show `099ccc8` or a
...
3. Prove `PYTHONPATH=src pytest -q tests/unit` -> expect **256 passed**,
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.19, this handoff,
...
**THE FIRST TASK IS THE NEXT GATE'S SCOPE MESSAGE — M4 (LLM
judge-assist) or r9 (event-date repair), operator's pick.** Each needs
its own scope -> operator "go" -> build; do not code before the go. r9
frees ~5,400 date-family rows from the M3 exclusion, so it likely pays
first; M4's open decisions are model choice and per-run cost cap
(GATEM proposal: current small model, cap 200 calls/run; proposer
only, never the verdict; acceptance gate against the judged rows).
...
1. **r9 event-date repair** (~5,400 rows) — rule 4.20 binding: read three
   real specimens before writing any regex; own gate, own commit.
2. **52 queued verdicts + the 5,563-entry M3 queue** — operator judges whenever; excluded rows are
   fine indefinitely.
3. **F2 (stated priorities)** — scoped, probe unrun; needs network and a
   quiet SEC window.
4. **fetch-probe 5 200** then `config.FETCH_POOL_ENABLED = True` — must
   not run alongside any other SEC job.
5. **M4 (LLM judge-assist)** — own scope-go-build; proposer only,
   never the verdict; acceptance gate against the judged rows before
   trust. Open decisions: model choice and per-run cost cap (proposal:
