docs/20260905_v26_Implementation_Plan.md

# Implementation plan and gate ledger

Bioindustry Intelligence Platform · v26 · 2026-09-05 (v26: F1 status updated — direction defect found via the stub list, table rebuilt with document-driven orientation, SEC-header verification running at 98.1% match; library ladder defect repaired; parallel fetch pool built behind a switch pending its probe. Prior header: v25 · 2026-09-03 (v25: ledger rows added for the store extension, the expert-hypothesis store, the constants annotation pass, the raw-SQL guard, the cancelled references rename, and the scoped threshold-reconciliation and F2 gates; fingerprint proofs for L4-P / store extension / annotation pass are owed together at F1 close. Prior header: v24 · 2026-09-03 (v24: gate L4-P DONE —
`aspect-match` parameter elimination. The two constants the module shipped
with at L4 (a 0.05 Jaccard cut, a single 5-year LOE horizon) were unsourced;
both are eliminated rather than re-tuned, because tuning them against the
same historical deals would fit on the test set. Therapeutic-area overlap
now contributes its raw Jaccard strength (no threshold); the LOE horizon is
reported as a sensitivity across 3/5/7/10 years with the pre-registered mark
applied at each. Aspect score is an equal-weight sum of strengths; the
equal-weight prior is unchanged. 191 tests, two of which assert the constants
cannot return. L4 results of record are unchanged and not re-run; the re-run
belongs to the aspect-match v2 pre-registration. Direction of record for v2,
from the M&A-prediction literature: temporal-graph link prediction over dated
firm edges, replacing hand-set aspect rules, blocked on F1 and F2 and scoped
with its own pre-registration.) Prior header: v23 · 2026-09-02 (v23: gate L4 DONE — `aspect-match` (aspects.py; `pairs-aspect [forward]`, 68th command): eight analyst-stated presence rules over the aspect vocabulary and the entity-attribute tables (equal weights, the naive prior; TA_JACCARD_MIN 0.05, LOE horizon 5y; stated aspects read equity_stakes/stated_priorities/assets as-of the cutoff, never the scored deal's own dossier; consideration_type recorded not-evaluable pre-announcement), rules derived from the Tempus record and the literature with the four Tempus-sequence deals excluded from both evaluations as the rules' source (P19). Paired beside mass-exact on 129 shared events/samples: HR@5 0.108 (±0.005) vs 0.335 (±0.014); pre-registered pass mark (HR@5 > 0.334 + 2× pooled sd 0.0107) fires NOT ADOPTED — mass-exact stands, the number is the finding. Forward test: 85 buyers, 1,389 buyer-years (613 degenerate propose nothing under midpoint ties), 123 evaluable deals, median pool 660; hits@5/10/25 = 8/11/17 vs chance 0.88/1.76/4.39; 5,921 top-10 false alarms; pool definition verbatim in the run note. Per-aspect coverage as ledger run metrics: three derived aspects carried everything; patent_cliff_pressure and all stated aspects evaluable 0 (zip absent; attribute tables empty before the cutoffs) — the matcher is substrate-starved, not rule-starved, and the a3 aspect-extraction rules plus 13D/13G ingestion are the named path. Incident R1 recorded: out-of-record `robust`/`improve` re-runs restored byte-exact from their original 2026-08-31 ledger records via P17 render-from-record. Order of record L1 → 1.4 → L2 → 2.10 → 1.5 → 2.9′ → L3 → L4 complete; next work items are decisions, PROJECT_STATUS §0.9. prior header: v22 · 2026-09-02 (v22: gate L3 DONE — the deal analyser (rule L3-a2): per-party EFTS collection of a deal's paper trail; span-grounded proposals for deal_terms/deal_timeline/deal_rationale over normalized captures; conflict-hold on same-field disagreements; verdict-gated consume (judged-wrong and unresolved conflicts never write; seed rows never overwritten). Yardstick vs the hand-built TEM-PSNL dossier: raw extraction precision 20/32 = 0.625 [0.453, 0.771], consumed 8 verdict-gated rows incl. two facts the seed lacked (exact termination fee $76,806,179; second-extension outside date 2028-04-20). Batch probe on ten never-seen historical deals: ~100 documents, 0 fetch failures, 100% span verification; spot-check on five deals (74 proposals): 26 correct / 31 wrong / 17 unsure = 0.456 raw, with correctness concentrated in deal 8-Ks/proxies and noise in 10-K risk boilerplate — the a3 rule agenda (doc-type guards, two-tier fee fields, price context guard) is written down, not built. v21: gate 2.9′ DONE — universe widening rule of record in universe.py (diagnostics/tools/data companies as plausible biopharma M&A parties; listed via `add`, private via `add --stub`); Tempus AI (IID 1379) and Personalis (IID 1380) admitted; the seed dossier's ma_events index row attached with both IIDs and deal_id TEM-PSNL-20260720 (L2 deferral closed; manual ma_events.* overrides now applied at every labels rebuild, keyed FilerTicker|AnnounceDate); measured effect: ranked universe 1,357→1,359, pool 862→863, hit@10 0.27 and hit@25 0.39 unchanged; targeted mine finding: diagnostics filings carry no PDUFA/readout language (0 hits over 2 companies) — the widened class contributes M&A structure, not FDA catalysts; live regression baselines re-hashed explicitly in the gate commit; v20: gate 1.5c DONE — realized events, delays and outcome states written from the candidate ledger; r10 rules (readout clause guard, runway guard with expectation exception, colon halves) measured on a fresh blind sample at 56/60 = 0.933 [0.841, 0.974], recall held 0.846/0.915; 2,053 realized rows (met_primary 24, not_met 41, unspecified 1,583 readouts + past exact PDUFA targets), 9 delay rows, 1 withdrawn guidance superseded; r11 candidates documented (half of calendar year, report-verb long window, undated readouts near trial starts); v19: gate 1.5b-eval DONE — measured improvement loop over the miner: candidate reviews table, cached re-runs, per-version precision with Wilson intervals, miss diagnostics with live EFTS probes, stale-row retirement; numbers of record at rule version r9: precision 56/60 = 0.933 [0.841, 0.974] from stored blind verdicts, recall 55/65 = 0.846 raw and 54/59 = 0.915 members-only, all ten misses classified (five non-members; five press-release-only, absent from EDGAR — the aggregator's own notes cite Business Wire), zero extraction failures among the misses, five true finds the benchmark lacks; r10 rules named for 1.5c (readout clause guard, through-the-data runway, 1H:YYYY colon form); v18: gate 1.5b DONE — SEC EDGAR full-text miner with a candidate ledger (every examined window kept with grounding offsets, TimeML-style value/precision/modifier, ConText-style assertion status, rule version); numbers of record precision 8/10 on the judged sample, recall 0.846 vs the FDA Tracker benchmark snapshot, rule version r6; gate 1.5c added next: realized rows, delay events and outcome states written from the ledger; v17: gate 1.5 split by decision 2026-08-31 into 1.5a DONE — schema forward columns, CT.gov primary-completion writer, FDA AdCom writer on the calendar's JSON endpoint — and 1.5b next: SEC EDGAR full-text miner for PDUFA dates and readout guidance, FDA Tracker as discovery layer (terms check first) and recall benchmark, precision by spot-check; testing standard adopted: parsers for external sources are built and tested against a real capture, never a synthetic fixture; v16: gate 2.10 DONE under the storage ruling — database sole store, no row content in git, snapshot the sole cross-machine reconciliation, manual export/load the rebuild path for all hand tables; v15: gate L2 DONE; membership check MEMBERSHIP_MATCHES 0 triggered the §4 conditional, so 2.9′ precedes L3 — order of record L1 → 1.4 → L2 → 2.10 → 1.5 → 2.9′ → L3 → L4; v14: decision — L2 loads its seed dossier through a committed hand-curated seed on the verified-overlay pattern, not the manual layer; gate 2.10 pulled forward to immediately after L2 so the correction machinery exists when the first corrections can arrive at L3; order of record now L1 → 1.4 → L2 → 2.10 → 1.5 → L3 → L4; v13: gate 1.4 DONE — event_date added to EVENT_TABLE_COLS by approved amendment, scheduled_date reserved for goal dates; v4 event classes declared at 1.4 so the vocabulary is complete from birth, writers at their own gates) · prior header: v12 · 2026-08-31 (v12: gate L1 DONE) · prior header: v11 · 2026-08-31 (v11: L1 rewritten to the approved file-room design, Ontology v5 §3.9; AI-over-store, Zotero-as-reader and video_dl recorded in §4 as deferred/policy items; v10: sequencing decided, §3; P19 Evidence binding for L1–L4 (Design Principles v5); order: L1 → 1.4 → L2 → 1.5 → L3 → L4, with 1.6, 1.7 and 2.9′ placed as they fall due; v9: dossier track L1–L4 and 2.9′ added per Ontology v4 §3.9–§3.10, §5.5–§5.7, §7; v8: data-snapshot policy decided, §4 and §5; v7: gate 0.4 DONE, Phase 0 complete; v6: gate 0.3 DONE; v5: gate 0.2 DONE; v4: Phase 0 reordered — storage migration to DuckDB as 0.2, reporting layer 0.3, model framework 0.4; legacy rule; thirteen-file regression baseline; v3: gate 0.1 DONE; v2: ontology roadmap steps mapped; gates M1-G/H/I added) · Status: agreed in
...
| L4-P | eliminate the two unsourced constants in aspect-match | L4 | audit finding | no threshold; LOE as sensitivity; L4 results untouched | DONE 2026-09-03 (7b2967d; fingerprint proof owed at F1 close) |
| STORE-X | `store.update_rows`, `store.add_columns`, identifiers from schema.py | 0.2 | reserved-word incident | no raw SQL in feeders; bad input raises | DONE 2026-09-03 (15c5ebc; fingerprint proof owed at F1 close) |
| HYP | expert-hypothesis store: table, entry, artifact-dated as_of, resolution, ledger | STORE-X | decisions of record | leak guard tested; recollected unscored; no expiry | DONE 2026-09-03 (1c9cb47; matcher wiring belongs to v2) |
| ANNOT | constants annotated with provenance; thresholds centralized, values unchanged; six docstring lies corrected | audit | audit v3–v5 | no behaviour change; suite green | DONE 2026-09-03 (3399234, fd00d64; fingerprint proof owed at F1 close) |
| SQL-GUARD | test fails the suite on SQL outside the store tier; references rename cancelled (P9) | STORE-X | sizing 29 sites / 8 modules | allowlist asserted exactly | DONE 2026-09-03 (058d729) |
| THRESH | measure the buyer/target size boundary and the token-floor sensitivity; one declared rule; re-based fingerprints recorded as deliberate | F1 close | docs/20260903_v1_Threshold_Reconciliation_Gate.md | selection rule pre-registered before evaluation | SCOPED |
| F1 | equity stakes from 13D/13G | 69bca93 | roadmap §F1 | rows at scale; direction verified vs SEC header; percent/date by second route; stubs by approval; fingerprints | IN PROGRESS — collector done; parser r7c; table rebuilt 41,137 rows with document-driven orientation (11,302 had been inverted); SEC-header verification 98.1% match at 57%; crosscheck and stubs commands built; close sequence in handoff v32 §2.3 |
| POOL | parallel SEC fetching under a 5/s aggregate ceiling (operator decisions 2026-09-05) | — | docs/20260905_v1_Parallel_Fetch_Scope.md | ceiling holds in every 1-s window; 429 halves; probe passes on real SEC before any loop uses it | BUILT, OFF (5cee57d, 69a0ee9); probe after the header check |
| F2 | stated priorities + deal aspects (analyser a3) | F1 | docs/20260903_v1_GATEF2_SCOPE.md | per-tier precision; coverage metrics | SCOPED; probe code written (058d729), not run |
| L4 | `aspect-match` implementation under `acquirer-pairing` (Ontology v4 §5.5) and the forward hit/false-alarm test (§5.6); pass mark written in the scope message | L3, 0.4 | L3 precision accepted | hits and false alarms per buyer-year against chance in a ledger record; `predict` unchanged until a separate composition decision | DONE 2026-09-02 (aspects.py; registry entries aspect-match [predict] + aspect-paired [evaluation]; paired 0.108 vs 0.335 NOT ADOPTED per the pre-registered mark; forward 8/11/17 hits vs 0.88/1.76/4.39 chance, 5,921 false alarms; coverage metrics per aspect in every run; Tempus-sequence exclusion in every run note; pytest 171; thirteen fingerprints MATCH; runbook docs/20260902_v2_GATEL4_INSTALL.md) |
