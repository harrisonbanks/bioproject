...
0e. docs/20260831_v15_Implementation_Plan.md — gate ledger and procedure; Phase 0, L1 and 1.4 DONE; order of record §3: L1 → 1.4 → L2 → 2.10 → 1.5 → L3 → L4, with 1.6, 1.7 and 2.9′ placed as they fall due; L2 DONE; next gate 2.10; 2.9′ precedes L3 (membership result); §4 snapshot of record; §5 thirteen-file baseline; §6 legacy register; §7 model naming.
...
3. **Gate 2.10 scope message (next).** Manual layer per plan row 2.10: manual_entities / manual_attributes / manual_notes, precedence, provenance, validation, load-time merge; plus the typed-edge columns on `relationships` deferred from 0.1. First consumer of corrections is L3's review queue. Standing capture offers (no gate needed): Tempus PR print-to-PDF + `library add <file> --for Rf573315b3de` re-admits equity_value_usd; merger-agreement exhibit or §9 news URLs re-admit the three Merck rows; remaining §9 URLs unlock further timeline/comparables rows. Q10 (transcript sources) remains open.
