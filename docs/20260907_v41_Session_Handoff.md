# docs/20260907_v41_Session_Handoff.md — paste this as the first message of the new chat

**Project:** Bioindustry Intelligence Platform. Repo `harrisonbanks/bioproject`
(private), branch `jason/refactor`, HEAD = `c489eb2` or a later commit the
...
1. Clone/pull; `git --no-pager log --oneline -1` must show `c489eb2` or a
...
3. Prove `PYTHONPATH=src pytest -q tests/unit` -> expect **259 passed**,
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.20, this handoff,
...
judge-assist, the last GATEM sub-gate) or F2 / fetch-probe, operator's
pick.** Each needs its own scope -> operator "go" -> build; do not
code before the go. M4's open decisions are model choice and per-run
cost cap (GATEM proposal: current small model, cap 200 calls/run;
proposer only, never the verdict; acceptance gate against judged rows,
now at rule version F1-r9).
...
2. **F2 (stated priorities)** — scoped, probe unrun; needs network and a
   quiet SEC window.
3. **fetch-probe 5 200** then `config.FETCH_POOL_ENABLED = True` — must
   not run alongside any other SEC job.
4. **M4 (LLM judge-assist)** — own scope-go-build; proposer only,
...
5. **Known gaps recorded, not built:** relation types beyond merged_into
