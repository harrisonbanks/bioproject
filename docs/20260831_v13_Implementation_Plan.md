...
adapter that 1.5 builds, so 1.5 precedes L3. L1 and 1.4 are DONE; next gate:
L2 — and its scope must resolve the standing dependency that L2's exit uses
the manual layer (gate 2.10, unbuilt): pull 2.10 forward or restate L2's exit.
