...
0e. docs/20260831_v13_Implementation_Plan.md — gate ledger and procedure; Phase 0, L1 and 1.4 DONE; order of record §3: L1 → 1.4 → L2 → 1.5 → L3 → L4, with 1.6, 1.7 and 2.9′ placed as they fall due; next gate L2 (whose scope resolves the 2.10 dependency); §4 snapshot of record; §5 thirteen-file baseline; §6 legacy register; §7 model naming.
...
3. **Gate L2 scope message (next).** Dossier schema per Ontology v5 §3.10 and the §3.2/§3.3 attributes; the scope must resolve the standing dependency first: L2's exit loads the hand-entered Tempus–Personalis dossier through the manual layer (gate 2.10), which is unbuilt — pull 2.10 forward or restate L2's exit without it. Open ontology questions Q9 (universe widening set; the one-query Tempus/Personalis membership check precedes 2.9′) and Q10 (transcript sources) remain.
