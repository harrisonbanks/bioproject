# docs/20260907_v44_Session_Handoff.md — paste this as the first message of the new chat
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.23, this handoff,
...
**THE FIRST TASK IS THE F2 COLLECTOR SCOPE (the two live tiers).**
F2 stage 2 is committed (594a63c, HEAD; suite 267): Q3 vocabulary +
Q2 columns at schema 0.18 (category CHECK deferred to consume-stage
mapping of the dossier seed's pre-Q3 values), Item-1 slicer
(last-heading boundary), five specimen-locked priority rules
(RULE_VERSION_F2 L3-a3-p1), earnings_call recorded as a coverage hole
(three dead query hypotheses). Before the collector runs at scale, fix
the two recorded drift defects in priorities.py: the per-hit
references/captures table-scan in _capture (build one index per pass,
the F1 2026-09-04 lesson) and the accession-first identifier ladder
that can attach a priorities capture to a mine-pdufa reference (the
§9 defect class; give priorities captures their own references, the
headers pattern). The writer stage must call add_columns for
source_type/section on the live stated_priorities table (the M1
disputed pattern). The fetch pool is ENABLED (probe 200/200, 0
backoffs, 4.1/s, run 20260907T171738).
