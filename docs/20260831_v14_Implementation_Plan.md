...
| 2.10 (B) | Manual layer: `manual_entities`, `manual_attributes`, `manual_notes` (template), precedence, provenance, validation, load-time merge; typed-edge columns on `relationships` (deferred from 0.1) | 0.2 | pulled forward 2026-08-31: runs immediately after L2, before 1.5 (first corrections arrive at L3) | a manual override is visible to a model with machine value retained; notes attach to entity and event; validate rejects a malformed row | PLANNED |
...
fall due. L2 loads its seed dossier through a committed hand-curated seed on
...
before L3's review queue produces the first corrections. Next gate: L2.
