...
diagrams: current state `docs/20260831_v2_System_Diagram.md` (Mermaid; v1 .png is the pre-Phase-0 record), target state `docs/20260830_v1_System_Diagram_TARGET_STATE.png`; plan and gate ledger: `docs/20260831_v8_Implementation_Plan.md`.
...
- **`data/snapshots/`** — `freeze` copies of the database file; the snapshot of
  record (2026-08-31, SHA-256 in PROJECT_STATUS 0.5) is shared between machines
  by USB, never through git.
...
`docs/regression_baseline.txt` holds thirteen SHA-256 fingerprints (seven live
exports regenerated per check, six legacy report files never regenerated); every
gate reproduces all thirteen byte for byte.
