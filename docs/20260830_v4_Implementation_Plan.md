...
Thirteen fingerprints replace the two-file baseline from gate 0.2 onward:
`ma_predictions.csv` plus the seven live report files (`pair_full_exact_report.txt`,
`robustness_report.txt`, `improve_report.txt`, `pair_exact_report.txt`,
`development_report.txt`, `tuning_report.txt`, `text_sweep_report.txt` — the last
absent where no 10-K text corpus exists) regenerated by the current code, and the
five legacy report files as they stand on disk (`fit_report.txt`, `holdout_report.txt`,
`pair_report.txt`, `pair_supervised_report.txt`, `pair_protocol_report.txt`,
`pair_substrate_report.txt`), never regenerated. Every gate from 0.2 must reproduce
each present file byte for byte; a file absent at baseline stays absent.
