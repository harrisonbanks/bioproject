# docs/20260909_v47_Session_Handoff.md — paste this as the first message of the new chat

**Project:** Bioindustry Intelligence Platform. Repo `harrisonbanks/bioproject`
(private), branch `jason/refactor`, HEAD = `03f8260` or a later commit the
...
1. Clone/pull; `git --no-pager log --oneline -1` must show `03f8260` or a
...
3. Prove `PYTHONPATH=src pytest -q tests/unit` -> expect **277 passed**,
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.26, this handoff,
...
**THE FIRST TASK IS THE TRIAGE COMMAND.** The F2 first-half measurement
is SEALED (e8504bd: 61/113 ~54% at L3-a3-p1, operator-adjudicated) and
the fix bundle is COMMITTED (f216973: verdict-aware write, 10
restorations applied, table 4,602 rows, max-tokens 1000, operator-only
precision). Build `priorities triage`: two model passes via the M4
machinery, agreements auto-recorded as reviewer="draft-agree",
the human receives ONLY disagreements plus a random audit slice, with
FULL VERBATIM sentences (never paraphrases - session lesson). Then the
deal-aspects half opens at rule version L3-a3-p2: commercial_infrastructure
enters the enum (decision record docs/20260908_v1_Vocabulary_Gap_
Commercial_Capability.md, banked specimens listed there), negation
guard + sentence-start capture land (Opus Scffcdfdcf539e686 and Biogen
S0f15a669a2338908 are the locked specimens), overflow rows become a
table, and the feed remeasures fresh. Also pending: the F1-era stakes
retire path (line ~2703) gets the relabel-style repair BEFORE the 52
deferred F-era verdicts are judged; successor-name stamping fixes via
aliases; the 707-entry M3 queue. Operator temperament note: hand him
NOTHING to read in bulk; disagreements only; verbatim always.
