# docs/20260911_v55_Session_Handoff.md — paste this as the first message of the new chat
# (supersedes v51/v52/v53/v54: v54 reused a filename mid-session, ruling 19 violation; content unchanged, renamed only)
...
`git --no-pager log --oneline -1` prints at boot, confirmed against
origin (Step 0) - never hardcoded in this file, since a docs commit
landing after this handoff was written would stale a fixed hash on
sight. As of this handoff's own last commit, HEAD was 6f74f86 (docs:
Manual S2.7 errata + scripts\ review note, following v1.30 at 520716c);
treat that as a floor, not the answer - if the pasted log shows
something later, that later commit is the real HEAD. Windows, root
...
2. Read IN FULL from the clone: docs/20260910_v2_Operating_Manual.md plus
   ...
   close, the R3 scope basis);
   docs/20260911_v1_Manual_Errata_S2_And_Scripts_Note.md (ruling 28, the
   no-directory-delete rule, and the scripts\ disposition); this
   handoff; docs/PROJECT_STATUS.md v1.30; the roadmap (5848d95), noting
   the trial record and the negative-result record both supersede its
   R2 call estimates.
...
6. `scripts\` folder (18 git-tracked files, unused, from the
   2026-08-29 src-layout migration): queued for review per
   docs/20260911_v1_Manual_Errata_S2_And_Scripts_Note.md. Options on
   record: archive (git mv to docs/archive/), delete with the file
   list in evidence, or confirm continued use.
