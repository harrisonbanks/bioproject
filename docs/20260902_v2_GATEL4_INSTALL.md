# docs/20260902_v2_GATEL4_INSTALL.md

# Gate L4 — aspect-match + forward hit/false-alarm test (install runbook, v2: exit evidence + incident R1)
...
## 5a. Exit evidence (operator pastes, 2026-09-02)
- Block L4-A: pytest 171 passed (99.13 s); ruff clean; `pairs-aspect` — 129
  events, aspect-match HR@5 0.108 (±0.005) / HR@10 0.155 vs MASS-exact 0.335
  (±0.014) / 0.411; pass mark → NOT ADOPTED (mass-exact stands, recorded
  finding); `pairs-aspect forward` — 85 buyers, 1,389 buyer-years (613
  degenerate), 123 evaluable deals, median pool 660, hits@5/10/25 = 8/11/17
  vs chance 0.88/1.76/4.39, false alarms 5,921; coverage: three derived
  aspects carried everything, patent_cliff_pressure and all stated aspects
  evaluable 0; untouched-surface diff empty.
- Incident R1: the L4-B block ran `robust` and `improve` beyond the
  five-command regeneration set of record (`predict; pairs-full-exact;
  pairs-exact; develop; develop tune`), overwriting two at-rest reports —
  eleven of thirteen MATCHed, isolating the artifact. Remedy (approved, three
  conditions): disposable `restore_reports.py` rendered each model's ledger
  records newest-first (cap 25, every attempt printed as TRIED), wrote only
  on a byte-exact hash match against docs/regression_baseline.txt, restored
  both files from their original 2026-08-31 runs (P17 render-from-record
  proven byte-exact in anger), was deleted in-block (Test-Path False), and
  the full verdict then read thirteen MATCH, PROBLEMS 0.
- Settled regression facts (standing): legacy six hash from
  data\gold_frozen_20260830\; all other baseline names from data\exports\;
  fixed-list routing, no fallback; five-command regeneration set; facts read
  from the committed record, never re-derived per block.
...
- Docs delivered with the gate commit: PROJECT_STATUS 1.10, plan v23,
  handoff v29, this runbook v2. (v1 named a README; none exists in the
  repository — corrected here.)
