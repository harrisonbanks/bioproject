# docs/20260907_v42_Session_Handoff.md — paste this as the first message of the new chat
...
4. Read in full: `docs/PROJECT_STATUS.md` v1.21, this handoff,
...
**THE FIRST TASK IS THE NEXT GATE'S SCOPE MESSAGE — F2 (stated
priorities) or fetch-probe, operator's pick.** Each needs its own
scope -> operator "go" -> build; do not code before the go. F2 needs
network and a quiet SEC window; fetch-probe must not run alongside any
other SEC job.
...
4. **Known gaps recorded, not built:** relation types beyond merged_into
