...
0e. docs/20260831_v16_Implementation_Plan.md — gate ledger and procedure; Phase 0, L1 and 1.4 DONE; order of record §3: L1 → 1.4 → L2 → 2.10 → 1.5 → L3 → L4, with 1.6, 1.7 and 2.9′ placed as they fall due; L2 and 2.10 DONE; next gate 1.5; 2.9′ precedes L3 (membership result); §4 snapshot of record; §5 thirteen-file baseline; §6 legacy register; §7 model naming.
...
3. **Gate 1.5 scope message (next).** Forward FDA calendar writers per plan row 1.5 and the FDA Catalyst design: SEC EDGAR full-text search for sponsor 8-K exhibits (PDUFA goal dates), the FDA AdCom calendar, and ClinicalTrials.gov primary-completion dates — writing forward rows (scheduled_date set, event_date blank) into events_table; the EDGAR full-text adapter built here is reused by L3. Standing, no gate needed: capture offers (Tempus PR --for Rf573315b3de re-admits equity_value_usd; merger-agreement exhibit or §9 news URLs re-admit the Merck rows; remaining §9 URLs unlock timeline/comparables rows); `manual export` before any rebuild-from-bronze, `manual load` after. Q10 (transcript sources) remains open.
