...
def main() -> None:
    for p, old, new in EDITS:
        ...
        assert s.count(old) == 1, f"{p.relative_to(ROOT)}: expected exactly one occurrence of the target line"
    ...
    R.check()
    print("Next: git add -A ; git commit -m \"Refactor step 1c: coverage manifest tag; portable VS Code venv path\"")
